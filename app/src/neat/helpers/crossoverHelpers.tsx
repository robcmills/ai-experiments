...
export function buildParent1(): Organism {
  ...
  network1.neuronMap.get(1).outputs.push(synapse6);
  neuron5.inputs.push(synapse6);
  network1.addSynapses([synapse4, synapse5, synapse6]);
  const genome1 = new Genome({ network: network1 });
  return new Organism({ fitness: 1, genome: genome1 });
}

export function buildParent2(): Organism {
  ...
  network2.neuronMap.get(3).outputs.push(synapse8);
  neuron5.inputs.push(synapse8);
  const synapse9 = new Synapse({
    from: network2.neuronMap.get(1),
    to: network2.neuronMap.get(6),
  });
  network2.neuronMap.get(1).outputs.push(synapse9);
  neuron6.inputs.push(synapse9);
  network2.addSynapses([
    synapse4,
    synapse5,
    synapse6,
    synapse7,
